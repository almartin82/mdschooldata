# ==============================================================================
# LIVE Pipeline Tests for mdschooldata
# ==============================================================================
#
...
test_that("Maryland DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Maryland DOE URL and verify HTTP 200")
})
...
test_that("Can download Maryland enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Maryland enrollment file", {
  skip_if_offline()
...
test_that("mdschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- mdschooldata::get_available_years()
  ...
  tryCatch({
    raw <- mdschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- mdschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- mdschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- mdschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- mdschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- mdschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- mdschooldata:::get_cache_path(2024, "enrollment")
