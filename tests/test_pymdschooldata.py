...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = md.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymdschooldata as md
        assert hasattr(md, 'fetch_enr')
        assert hasattr(md, 'fetch_enr_multi')
        assert hasattr(md, 'tidy_enr')
        assert hasattr(md, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pymdschooldata as md
        assert hasattr(md, '__version__')
        assert isinstance(md.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = md.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        """Total enrollment is in a reasonable range for Maryland."""
        import pymdschooldata as md
        years = get_test_years()
        df = md.fetch_enr(years['max_year'])
        ...
        if 'is_state' in df.columns and 'grade_level' in df.columns and 'subgroup' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Maryland should have around 880,000 students
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = md.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = md.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pymdschooldata as md
        years = get_test_years()
        df_single = md.fetch_enr(years['max_year'])
        df_multi = md.fetch_enr_multi([years['max_year']])
...
class TestDataIntegrity:
    ...
    def test_district_count_reasonable(self):
        """Maryland should have 24 local school systems."""
        import pymdschooldata as md
        years = get_test_years()
        df = md.fetch_enr(years['max_year'])
        if 'is_district' in df.columns and 'district_name' in df.columns:
            districts = df[df['is_district'] == True]['district_name'].nunique()
            # Maryland has exactly 24 LSS (one per county + Baltimore City)
            assert districts >= 20
            assert districts <= 30
